...
for f=1:size(mile_fraction,1)
    ...
    mph_mile_orig(f,:) = 23.3 - 2.*boxed_grade(f,:);
end

sections = 20;
sec_wid = size(mph_mile_orig,2)/sections;
for i=1:sections
    mph_adj((i-1)*sec_wid+1 : i*sec_wid) = 1 - (i*4/sections);
end
mph_mile = mph_mile_orig + mph_adj;

mins_mile = 60 ./ mph_mile(1,1:floor(max(run_total)));

%calculate distance vs time
for mile=1:size(mins_mile,2)
    if isnan(mins_mile(mile))==1
        mins_mile(mile) = mins_mile(mile-1);
    end
    if mile==1
        mins_total(mile) = mins_mile(mile);
    else
        mins_total(mile) = mins_total(mile-1) + mins_mile(mile);
    end
    min_at_mile( round(mins_total(mile)) ) = mile;
end

printout = 'hour,mile';
M2 = cell(size(min_at_mile,2)/60,3);
M2{1,1} = 0;
M2{1,2} = 16;
M2{1,3} = 0;
M2{1,4} = strcat(time_station_names.Name(1));
for hr=1:size(min_at_mile,2)/60,
    eastern_time(hr) = mod(16+hr,24);
    hour_at_mile(hr) = min_at_mile( 60*hr );
    k=1;
    while(hour_at_mile(hr)==0)
        hour_at_mile(hr) = min_at_mile( (60*hr) + k );
        k = k + 1;
    end
    M2{hr+1,1} = hr;
    M2{hr+1,2} = eastern_time(hr);
    M2{hr+1,3} = hour_at_mile(hr);
    printout = strcat(printout,num2str(hr),',',num2str(hour_at_mile(hr)),'\n'); 
    %set default
    M2{hr+1,4} = strcat(time_station_names.Name(size(time_station_names.Name,1)),' + ',num2str( hour_at_mile(hr)-time_stations(size(time_stations,1)) ),'mi');
    for k=1:size(time_stations,2),
        if time_stations(k) > hour_at_mile(hr),
            M2{hr+1,4} = strcat(time_station_names.Name(k-1),' + ',num2str( hour_at_mile(hr)-time_stations(k-1) ),'mi');
            break
        end
    end
end
%sprintf(printout)
dlmcell('hour_mile.csv',M2,',');

figure
subplot(2,1,1), plot(run_total, elev),xlabel('mile'), ylabel('elevation')
grid on
grid minor
subplot(2,1,2), plot( mph_mile(1,:) ), xlabel('mile'), ylabel('speed (mph)')
ylim([0 40]);
grid on
grid minor
% subplot(4,1,3), plot( mins_mile )
% subplot(4,1,4), plot( hour_at_mile )
return
