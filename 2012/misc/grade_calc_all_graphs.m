...
run_total_m(i_start-1) = 0;

% ----------
% calculate rise, run, and grade between each GPS waypoint
% ----------
i = 2;
for csvrow=i_start:i_end,
    %elevation in feet
    elev(i) = M(csvrow,3)*3.2808399;
    %run between waypoint in meters
    run(i) = gps_convert(M(csvrow-1,1), M(csvrow,1), M(csvrow-1,2), M(csvrow,2), M(csvrow,3));
    %rise between waypoints in meters
    rise(i) = M(csvrow,3) - M(csvrow-1,3);
    
    %-----for top elev graph
    elev_m(csvrow) = elev(i);
    %total distance on course in meters
    if run(i)<=0
        run_total_m(csvrow) = run_total_m(csvrow-1) + 0.0001;
    else
        run_total_m(csvrow) = run_total_m(csvrow-1) + run(i);
    end
    
    %if distance between waypoints > 40 meters, interpolate
    if run(i) > 40
        div = ceil(run(i)/40);
        ...
        for k=1:div,
            ...
            %total distance on course in meters
            if run(i)<=0
                run_total(i) = run_total(i-1) + 0.0001;
            else
                run_total(i) = run_total(i-1) + run(i);
            end
            %grade since last waypoint
            grade(i) = 100*rise(i)/run(i);
    ...
    if i_ts <= size(time_stations_m,2)
        if run_total(i) > time_stations_m(i_ts)
            i_ts = i_ts + 1;
            time_station_row(i_ts) = csvrow;
            time_station_i(i_ts) = i;
        end
    end
    i = i + 1;
end
...
%size(time_station_i,2)-1
for i_ts=1:size(time_station_i,2)-1,
    %start .csv row for this timestation
    i_start = time_station_i(i_ts);
    %ending .csv row for this timestation
    i_end = time_station_i(i_ts+1);
    ...
    % ----------
    h=figure;
    m_start = time_station_row(i_ts);
    m_end = time_station_row(i_ts+1);
    x_m = run_total_m(m_start:m_end) - run_total_m(m_start);
    ts_miles_m = run_total_m(m_end)-run_total_m(m_start);
    sp(1) = subplot(num_graphs,1,1);
    plot(x_m, elev_m(m_start:m_end), 'LineWidth', 1);
    title(strcat(time_station_names.Name(i_ts),{'  -  '},time_station_names.Name(i_ts+1),{'.   '},num2str(ts_miles,'%.1f'),{' miles.'}));
    %define axis, etc.
    %xlabel('TS mile');
    ylabel('elevation (ft)');
    xlim([0 ts_miles_m]);
    elev_min = min(elev_m(m_start:m_end));
...
%     set(sp(num_graphs),'Position',pos(num_graphs,:));
    ...
    savefig(strcat('graphs\all\TS',num2str(i_ts)), 'pdf', '-r300')
    %close window
    close(h)
end